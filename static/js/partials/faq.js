...
const questions_and_answers = [
  [
    "WHAT IS TAMU DATATHON", "A datathon is where you build your analytics skill set and create data driven solutions in 24 hours. We provide data science lectures, workshops, challenges, prizes, fun activities, swag, and more. We’ll take care of you while you learn & create!"
  ],
  [
    "WHERE IS THE EVENT", "Datathon will be hybrid (Both in-person and virtual)! If you are coming in-person, the event will be hosted at the Hall of Champions in Kyle Field."
  ],
  [
    "WHO CAN ATTEND", "TAMU Datathon is open to any enrolled undergraduate or graduate student who is at least 18 years of age, as well as people who have graduated within 1 year from the event. We welcome students from all across the world and from all majors!"
  ],
  [
    "HOW MUCH DOES IT COST", "It’s FREE! All you need is a laptop! (and Wi-Fi for virtual participants)"
  ],
  [
    "HOW MUCH DO I NEED TO KNOW", "None! If you’re a beginner to data science, TAMU Datathon is the perfect time and place to learn. We’ll provide introductory coursework and mentors to guide you along your journey to complete a data science project.\nFor our more advanced students, our challenges will peak your interest and allow you to put your skills to the test. We’re committed to helping you build something you can be proud of!"
  ],
  [
    "WHEN IS THE EVENT", "October 16-17th, 2021. We will publish a complete schedule in the fall."
  ],
  [
    "HOW DO I REGISTER", "Registration will open on August 1st, 2021 and will close on October 1st, 2021. Admission decisions will be released shortly after the registration period closes."
  ],
  [
    "HAVE OTHER QUESTIONS?", "October 16-17th, 2021. We will publish a complete schedule in the fall."
  ]
];

const sectionwrapper = document.getElementById("during");
const section_template = Handlebars.compile(`<div class="flex-container">
...
const empty = Handlebars.compile(`<div id=q{{0}}l{{1}} style="display: inline-block; margin-top:0.35; width: 0.8em; height: 1.6em;" class="empty"></div>`);
const mydiv_template = Handlebars.compile(`<div class="noselect" style="display: inline-block" id="q{{3}}l{{4}}">
...
for (var i = 0; i < questions_and_answers.length; i += 2) {
  ...
  questions_and_answers[i][0].split(" ").forEach((jj, m) => {
    ...
    for (var j = 0; j < jj.length; j++) {
      ...
      if (mydiv_input[2] > -0.7 && mydiv_input[2] < 0.7)
        mydiv_input[2] -= 2;
      mydiv_input[3] = i;
      mydiv_input[4] = k++;
...
document.querySelectorAll(".wrapper").forEach(evt => {
  evt.addEventListener("mouseover", e => {
    Array.from(document.getElementById(`${evt.id}`).getElementsByClassName("tile")).forEach(thing => {
...
function createfunction(i, j) {
  return function() {
    window.addEventListener("resize", () => {
      setTimeout(function() {
        let rect0 = document.getElementById(`q${i}l${j - 1}`).getBoundingClientRect();
        let rect1 = document.getElementById(`q${i}l${j}`).getBoundingClientRect();
        let rect2 = document.getElementById(`q${i}l${j + 1}`).getBoundingClientRect();
        if (Math.abs(rect1.top - rect2.top) > 8 || Math.abs(rect0.top - rect1.top) > 8) {
